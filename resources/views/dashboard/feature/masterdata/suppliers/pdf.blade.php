    ...
    <table>
        ...
        <tbody>
            @foreach ($data as $index => $item)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td> {{ \Carbon\Carbon::parse($item->created_mdsupplier)->locale('id')->translatedFormat('d F Y') }}
                    </td>
                    <td>{{ $item->name }}</td>
                    <td>{{ $item->code_mdsupplier }}</td>
                    <td>{{ $item->name_mdsupplier }}</td>
                    <td>{{ $item->address_mdsupplier }}</td>
                    <td>{{ $item->email_mdsupplier }}</td>
                    <td>{{ $item->phone_mdsupplier }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
